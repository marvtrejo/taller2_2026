---
title: "Clase 03: Álgebra de funciones"
...
ggplot(data.frame(x = c(0.01, 10)), aes(x = x)) +
  ...
  annotate("text", x = 2, y = -3, label = "Asíntota x = 0", color = "gray40", size = 3.5) +
  theme_minimal() + 
  labs(x = "x", y = "f(x)")
```
...
ggplot(data.frame(x = c(-3, 3)), aes(x = x)) +
  ...
  annotate("text", x = -1.5, y = 2, label = "Asíntota y = 0", color = "gray40", size = 3.5) +
  theme_minimal() + 
  labs(x = "x", y = "f(x)")
```
:::
...
2. **Diferencia (Resta):** $$(f - g)(x) = f(x) - g(x)$$
3. **Producto (Multiplicación):** $$(f \cdot g)(x) = f(x) \cdot g(x)$$
4. **Cociente (División):** $$\left(\frac{f}{g}\right)(x) = \frac{f(x)}{g(x)}, \quad \text{con } g(x) \neq 0$$
...
## ¡Atención con el dominio de la división!
Para el cociente de funciones, el dominio es la intersección de los dominios originales **excluyendo** los valores donde la función del denominador se hace cero ($g(x) = 0$). 

Dado que $g(0) = \sqrt{0} = 0$, debemos sacar al cero de nuestro dominio base. El corchete en el cero se convierte en un paréntesis.
:::
...
#### 4. División $(f/g)(x)$
Aplicamos la ley del sándwich (o extremos por extremos y medios por medios) para simplificar la fracción compleja:
$$\left(\frac{f}{g}\right)(x) = \frac{x^2}{\frac{1}{\sqrt{x}}} = \frac{\frac{x^2}{1}}{\frac{1}{x^{1/2}}} = x^2 \cdot x^{1/2} = x^{2 + 1/2} = x^{5/2} = \sqrt{x^5} = x^2\sqrt{x}$$

::: {.callout-note icon=false}
## Análisis del dominio en esta división
Recordemos que para el cociente de funciones, la regla general dice que el dominio es la intersección de los dominios originales **excluyendo** los valores donde la función del denominador se hace cero ($g(x) = 0$). 

Sin embargo, en este caso particular, la función $g(x) = \frac{1}{\sqrt{x}}$ **nunca** es igual a cero para ningún valor real de $x$ (ya que el numerador es una constante $1$). Por lo tanto, no tenemos que excluir ningún valor adicional de nuestra intersección original.
:::
...
**Definición:**
Dadas dos funciones $f$ y $g$, la función compuesta $f \circ g$ (se lee "$f$ composición $g$") está definida por:
$$(f \circ g)(x) = f(g(x))$$
...
Sustituimos la función $g(x)$ dentro de $f(x)$:
$$
\begin{aligned}
(f \circ g)(x) &= f(x^2 + 1) \\
&= \sqrt{x^2 + 1}
...
#### 2. $(g \circ f)(x)$
Sustituimos la función $f(x)$ dentro de $g(x)$:
$$
\begin{aligned}
(g \circ f)(x) &= g(\sqrt{x}) \\
&= (\sqrt{x})^2 + 1 \\
...
#### **1. $(f \circ g)(x)$:**
Tomamos la función $f$ y sustituimos cada '$x$' por la función $g(x)$ completa.
$$
\begin{aligned}
(f \circ g)(x) &= f(g(x)) \\
&= f(x - 3) \\
...
\end{aligned}
$$
*(Desarrollando el binomio, esto es igual a $x^2 - 6x + 9$)*.

#### **2. $(g \circ f)(x)$:**
Tomamos la función $g$ y sustituimos cada '$x$' por la función $f(x)$.
$$
\begin{aligned}
(g \circ f)(x) &= g(f(x)) \\
&= g(x^2) \\
...
**1. La función más interna**
Partimos de una "función base" o la función más interna, en este caso es $h(x)$:
$$h(x) = x + 3$$

**2. Primera composición $g(h(x))$**
Tomamos la función intermedia $g(x)$ y sustituimos la variable $x$ por toda la expresión de $h(x)$:
$$
\begin{aligned}
g(h(x)) &= g(x+3) \\
&= (x+3)^2 
\end{aligned}
$$
Si desarrollamos el binomio al cuadrado, obtenemos la expresión equivalente: $x^2 + 6x + 9$.

**3. Composición final $f(g(h(x)))$**
Finalmente, tomamos la función externa $f(x)$ y sustituimos cada '$x$' en su fórmula por el resultado completo que obtuvimos en el paso anterior:
$$
\begin{aligned}
f(g(h(x))) &= f((x+3)^2) \\
&= \frac{(x+3)^2}{(x+3)^2 + 1}
...
Para presentar el resultado de una forma más limpia, utilizamos la forma desarrollada del binomio y sumamos los términos semejantes en el denominador:
$$
\begin{aligned}
f(g(h(x))) &= \frac{x^2 + 6x + 9}{(x^2 + 6x + 9) + 1} \\
&= \frac{x^2 + 6x + 9}{x^2 + 6x + 10}
\end{aligned}
$$
